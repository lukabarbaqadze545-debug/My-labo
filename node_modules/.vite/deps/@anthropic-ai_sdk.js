...
import { a as AnthropicError, c as ConflictError, d as PermissionDeniedError, f as RateLimitError, g as isAbortError, h as castToError, i as APIUserAbortError, l as InternalServerError, m as UnprocessableEntityError, n as APIConnectionTimeoutError, o as AuthenticationError, p as RetryableError, r as APIError, s as BadRequestError, t as APIConnectionError, u as NotFoundError } from "./error-CgsyLo7K.js";
...
async function checkCredentialsFileSafety(path, onWarn = (m) => console.warn(`anthropic-sdk: ${m}`)) {
	...
	const { fs } = await import("./node.browser-CTzfjKhr.js");
	let resolved = path;
	let st;
	try {
...
async function writeCredentialsFileAtomic(targetPath, data) {
	const { fs, path } = await import("./node.browser-CTzfjKhr.js");
...
var getCredentialsPath = async (config, profile) => {
	...
	validateProfileName(profileName);
	const { path } = await import("./node.browser-CTzfjKhr.js");
		...
		try {
			resp = await config.fetch(url, {
				...
				headers: {
					...
					"User-Agent": config.userAgent || `anthropic-sdk-typescript/0.123.0 oidcFederationProvider`
				},
				body: JSON.stringify(body)
			});
		} catch (err) {
		...
		try {
			resp = await config.fetch(url, {
				...
				headers: {
					...
					"User-Agent": config.userAgent || `anthropic-sdk-typescript/0.123.0 userOAuthProvider`
				},
				body: JSON.stringify(body)
			});
		} catch (err) {
...
_EnvironmentWorker_signal = /* @__PURE__ */ new WeakMap(), _EnvironmentWorker_instances = /* @__PURE__ */ new WeakSet(), _EnvironmentWorker_handleItem = async function _EnvironmentWorker_handleItem(work, environmentKey, externalSignal) {
	...
	const agentToolset = await import("./node.browser-CPMHaqNd.js");
	let leaseTtlMs;
	let runner;
	...
	[(_BetaMessageStream_currentMessageSnapshot = /* @__PURE__ */ new WeakMap(), _BetaMessageStream_params = /* @__PURE__ */ new WeakMap(), _BetaMessageStream_connectedPromise = /* @__PURE__ */ new WeakMap(), _BetaMessageStream_resolveConnectedPromise = /* @__PURE__ */ new WeakMap(), _BetaMessageStream_rejectConnectedPromise = /* @__PURE__ */ new WeakMap(), _BetaMessageStream_endPromise = /* @__PURE__ */ new WeakMap(), _BetaMessageStream_resolveEndPromise = /* @__PURE__ */ new WeakMap(), _BetaMessageStream_rejectEndPromise = /* @__PURE__ */ new WeakMap(), _BetaMessageStream_listeners = /* @__PURE__ */ new WeakMap(), _BetaMessageStream_ended = /* @__PURE__ */ new WeakMap(), _BetaMessageStream_errored = /* @__PURE__ */ new WeakMap(), _BetaMessageStream_aborted = /* @__PURE__ */ new WeakMap(), _BetaMessageStream_catchingPromiseCreated = /* @__PURE__ */ new WeakMap(), _BetaMessageStream_response = /* @__PURE__ */ new WeakMap(), _BetaMessageStream_request_id = /* @__PURE__ */ new WeakMap(), _BetaMessageStream_workspace_id = /* @__PURE__ */ new WeakMap(), _BetaMessageStream_logger = /* @__PURE__ */ new WeakMap(), _BetaMessageStream_handleError = /* @__PURE__ */ new WeakMap(), _BetaMessageStream_instances = /* @__PURE__ */ new WeakSet(), _BetaMessageStream_getFinalMessage = function _BetaMessageStream_getFinalMessage() {
		...
		switch (event.type) {
			...
			case "message_delta":
				...
				if (event.input_transformations != null) snapshot.input_transformations = event.input_transformations;
...
var APIKeys = class extends APIResource {
	...
	*/
	update(apiKeyID, body, options) {
		return this._client.post(path`/v1/organizations/api_keys/${apiKeyID}?beta=true`, {
			body,
			...options
		});
	}
...
//#endregion
//#region node_modules/@anthropic-ai/sdk/resources/beta/organization/compliance-settings.mjs
var ComplianceSettings = class extends APIResource {
	...
	*/
	update(body, options) {
		return this._client.post("/v1/organizations/compliance_settings?beta=true", {
			body,
			...options
		});
	}
...
var Users = class extends APIResource {
	...
	*/
	update(userID, body, options) {
		return this._client.post(path`/v1/organizations/users/${userID}?beta=true`, {
			body,
			...options
		});
	}
...
var Members = class extends APIResource {
	...
	*/
	list(workspaceID, query = {}, options) {
		return this._client.getAPIList(path`/v1/organizations/workspaces/${workspaceID}/members?beta=true`, Page, {
			query,
			...options
		});
	}
...
var RateLimits = class extends APIResource {
	...
	*/
	list(workspaceID, query = {}, options) {
		return this._client.getAPIList(path`/v1/organizations/workspaces/${workspaceID}/rate_limits?beta=true`, PageCursor, {
			query,
			...options
		});
	}
...
Anthropic.Beta = Beta;
//#endregion
//#region node_modules/@anthropic-ai/sdk/lib/middleware.mjs
var encoder = new TextEncoder();
/** Betas sent by default; override with {@link BetaRefusalFallbackOptions.betas}. */
var DEFAULT_BETAS = ["fallback-credit-2026-07-01"];
/**
...
function applyFallbackPatch(body, entry) {
	const patched = { ...body };
	for (const [key, value] of Object.entries(entry)) if (key === "output_config" && value != null) {
		const merged = { ...patched[key] ?? {} };
		for (const [subKey, subValue] of Object.entries(value)) patchField(merged, subKey, subValue);
		patchField(patched, key, Object.keys(merged).length ? merged : null);
	} else patchField(patched, key, value);
	return patched;
}
...
function betaRefusalFallbackMiddleware(fallbacks, options = {}) {
	...
	return async (request, next, ctx) => {
		...
		if (fallbacks.length === 0 || ctx.options?.method !== "post" || path !== "/v1/messages" || new URLSearchParams(query).get("beta") !== "true" || typeof ctx.options.body !== "object" || ctx.options.body == null) return next(request);
		if (ctx.options.body.fallbacks != null) throw new AnthropicError("Sending the `fallbacks:` request param is not supported when using the `betaRefusalFallbackMiddleware`. You should either remove the middleware and send `fallbacks:` with the `server-side-fallback-2026-07-01` beta header to let the API handle refusal fallbacks, or omit the `fallbacks:` param if you'd like `betaRefusalFallbackMiddleware` to handle fallbacks on the client side.");
		...
		if (!Number.isInteger(startIndex) || startIndex < -1 || startIndex >= fallbacks.length) throw new AnthropicError(`fallbackState.index ${startIndex} is out of bounds for a chain of ${fallbacks.length} fallback(s); was the state shared with a different middleware?`);
		const pin = (index) => {
		...
		if (ctx.options.stream === true) {
			...
			if (firstHop >= fallbacks.length || typeof initialRequest.body !== "string") return response;
		...
		while (index < fallbacks.length - 1) {
			...
			if (message?.type !== "message" || message.stop_reason !== "refusal") break;
			index += 1;
			pin(index);
		...
		if (fallbackBlocks.length === 0) return res;
		const served = await ctx.parse(res);
		if (served?.type !== "message" || served.stop_reason === "refusal" || !Array.isArray(served.content)) return res;
...
function spliceFallbackStream(args) {
	...
	else signal?.addEventListener("abort", makeAbort(controller, signal), { once: true });
	...
	for (let hop = firstHop; hop < fallbacks.length; hop++) {
		...
		if (failure) {
			...
			yield emit("message_delta", {
				...
				usage: lastUsage ?? {},
				...refusalInputTransformations !== void 0 && { input_transformations: refusalInputTransformations }
			});
		...
		switch (p?.type) {
			...
			case "message_delta":
				if (p.delta.stop_reason === "refusal") {
					...
					if (!details?.fallback_credit_token) onError({
				...
				if (splice) {
					...
					p.usage = usage;
					if (!("input_transformations" in p) && startInputTransformations !== void 0) p.input_transformations = startInputTransformations;
...
var BlockTracker = class {
	...
	*closeOpenBlocks() {
		for (const index of this.open) yield emit("content_block_stop", {
			type: "content_block_stop",
			index
		});
		this.open.length = 0;
...
function buildFallbackRequest(orig, { model, creditToken, continuation }) {
	...
	if (continuation.length) body.messages = [...body.messages, {
		role: "assistant",
...
function applyDelta(blocks, index, delta) {
	...
	switch (delta.type) {
		...
		case "thinking_delta":
			...
			break;
		case "signature_delta": block.signature = delta.signature;
	}
}
/**
...
function withMiddlewareHeaders(request, betas) {
	...
	for (const beta of betas) if (!existing.has(beta)) {
...
function backfill(primary, fallback) {
	...
	for (const k of Object.keys(out)) if (out[k] == null && fallback?.[k] != null) out[k] = fallback[k];
	return out;
}
/**
...
//#endregion
export { AI_PROMPT, APIConnectionError, APIConnectionTimeoutError, APIError, APIPromise, APIUserAbortError, Anthropic, Anthropic as default, AnthropicError, AuthenticationError, BadRequestError, BaseAnthropic, BetaFallbackState, ConflictError, HUMAN_PROMPT, InternalServerError, NotFoundError, PagePromise, PermissionDeniedError, RateLimitError, RetryableError, UnprocessableEntityError, betaRefusalFallbackMiddleware, MIN_MEMORY_SYNC_INTERVAL_MS as t, toFile };
